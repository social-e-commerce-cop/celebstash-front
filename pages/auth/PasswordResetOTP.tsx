...
type PasswordResetOTPScreenNavigationProp = StackNavigationProp<AppStackParamList, 'PasswordResetOTP'>;
type PasswordResetOTPScreenRouteProp = StackScreenProps<AppStackParamList, 'PasswordResetOTP'>['route'];
...
const PasswordResetOTP: React.FC = () => {
  ...
  const { initiatePasswordReset, verifyPasswordResetOtp, isLoading } = useAuth();
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      const response = await verifyPasswordResetOtp(otpData);
      
      if (response.success) {
        // OTP is valid, navigate to create password screen with the verified OTP
      ...
      <View style={styles.otpContent}>
        ...
        <View style={styles.formSection}>
          ...
          <View style={styles.mb8}>
            <Text style={styles.formHeaderTitle}>Verify Reset Code</Text>
          ...
          <TouchableOpacity
            style={[styles.verifyBtn, isLoading || code.some((digit) => !digit) ? styles.verifyBtnDisabled : null]}
